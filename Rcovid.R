...
#i set up the directory in which im goint to work 
...
#i will set my DB to a tibble, to create a filter and work only with a lil part of the dataset.    
#N1 <- as_tibble(M) 
#N2<- N1 %>% filter(PY >=2018) 
#i set it up again into a data set but now i work with the filter (just to make the analyses faster) 
#M1 <- as.data.frame(N2)  
...
# Conceptual Structure using keywords (method="CA") 
#This is the cluster chapter, i think this is one of the most important parts of the analyse.
t<-as.tibble(M) 
ti<- t %>% filter(PY==2021) 
...
write.table(S, file="Modelo2.csv", sep=";", row.names= T)
####################################################################
###Models based on S2 which is a variable for bibliometrix###
####################################################################
attach(S2)
...
matrix=as.matrix(letras) 
'''i will order and summ the letters in the matrix''' 
vector <- sort(rowSums(matrix),decreasing=TRUE)  
'''i create the data with the frecuency''' 
dataletras <- data.frame(word= names(vector),frequencia=vector)  
################ word frecuency plots 
